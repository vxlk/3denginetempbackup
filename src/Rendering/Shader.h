...
namespace Shaders
{
	...
	public:
		...
		const VertexShaderGLSL			NULL_VERT_SHADER;
		const FragmentShaderGLSL		NULL_FRAG_SHADER;
		static const GLuint			    INVALID_PROGRAM = -1;

		const VertexShaderGLSL&   getVertexShaderByName(const std::string& name);
		const FragmentShaderGLSL& getFragmentShaderByName(const std::string& name);
